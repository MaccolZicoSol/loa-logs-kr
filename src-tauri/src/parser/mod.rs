...
pub fn start_beta(
    ...
    let rx = match read_capture(port, region_file_path.to_string(), packets_file_path.to_string()) {
        Ok(rx) => rx,
        Err(e) => {
            warn!("Error starting capture: {}", e);
            return Ok(());
    ...
    while let Ok((op, data, time)) = rx.recv() {
        if reset.load(Ordering::Relaxed) {
            state.soft_reset(true);
            reset.store(false, Ordering::Relaxed);
        }
        ...
        let now_instant = Instant::now();
        {
            ...
            let secs = time / 1000;
            let nanos = (time % 1000) * 1_000_000;
    
            let target_time_opt = Utc.timestamp_opt(secs, nanos as u32);
    
            let target_time = match target_time_opt.single() {
                Some(t) => t,
                None => {
                    info!("Error: Invalid timestamp");
                    continue;
                }
            };
    
            let duration = now_utc.signed_duration_since(target_time);
    
            if duration.num_seconds() < 0 {
                now_instant - std::time::Duration::from_secs(duration.num_seconds().abs() as u64)
            } else {
                now_instant + std::time::Duration::from_secs(duration.num_seconds() as u64)
            };
        }
        let now_time = Utc.timestamp_millis_opt(time).unwrap();
        ...
        match op {
            ...
            Pkt::SkillStartNotify => {
                if let Some(pkt) = parse_pkt(&data, PKTSkillStartNotify::new, "PKTSkillStartNotify")
                {
                    ...
                    let timestamp = now_time.timestamp_millis();
                    let (skill_id, summon_source) = state.on_skill_start(
                        &entity,
                        pkt.skill_id,
                        tripod_index,
            ...
            // }
            Pkt::SkillDamageAbnormalMoveNotify => {
                if !read_packet && now_instant - raid_end_cd < Duration::from_secs(10) {
                    debug_print(format_args!(
                        "ignoring damage - SkillDamageAbnormalMoveNotify"
                    ));
                    continue;
                ...
                if let Some(pkt) = parse_pkt(
                    ...
                    let now = now_time.timestamp_millis();
                    let owner = entity_tracker.get_source_entity(pkt.source_id);
                    let local_character_id = id_tracker
                        .borrow()
                        .get_local_character_id(entity_tracker.local_entity_id);
