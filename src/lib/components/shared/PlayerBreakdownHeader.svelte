...
{#if hasFrontAttacks && meterSettings.breakdown.frontAtk}
    ...
    <th class="w-14 font-normal" use:tooltip={{ content: "% Damage from Front Attack" }}>F.A</th>
    {:else}
...
{#if hasBackAttacks && meterSettings.breakdown.backAtk}
    {#if meterSettings.positionalDmgPercent}
    <th class="w-14 font-normal" use:tooltip={{ content: "% Damage from Back Attack" }}>B.A</th>
    {:else}
...
{#if anySupportBuff && meterSettings.breakdown.percentBuffBySup}
    <th class="w-14 font-normal" use:tooltip={{ content: "% Damage buffed by Support Atk. Power Buff" }}>Buff%</th>
{/if}
{#if anySupportBrand && meterSettings.breakdown.percentBrand}
    <th class="w-14 font-normal" use:tooltip={{ content: "% Damage buffed by Brand" }}>B%</th>
{/if}
{#if anySupportIdentity && meterSettings.breakdown.percentIdentityBySup}
    <th class="w-14 font-normal" use:tooltip={{ content: "% Damage buffed by Support Identity" }}>Iden%</th>
{/if}
{#if meterSettings.breakdown.avgDamage}
    <th class="w-14 font-normal" use:tooltip={{ content: "Skill Average Damage per Hit" }}>APH</th>
{/if}
{#if meterSettings.breakdown.avgDamage}
    <th class="w-14 font-normal" use:tooltip={{ content: "Skill Average Damage per Cast" }}>APC</th>
{/if}
{#if meterSettings.breakdown.maxDamage}
    <th class="w-14 font-normal" use:tooltip={{ content: "Skill Max Hit Damage" }}>MaxH</th>
{/if}
{#if meterSettings.breakdown.maxDamage}
    <th class="w-14 font-normal" use:tooltip={{ content: "Skill Max Cast Damage" }}>MaxC</th>
{/if}
{#if meterSettings.breakdown.casts}
    <th class="w-12 font-normal" use:tooltip={{ content: "Total Casts" }}>Casts</th>
{/if}
{#if meterSettings.breakdown.cpm}
    <th class="w-12 font-normal" use:tooltip={{ content: "Casts per minute" }}>CPM</th>
{/if}
{#if meterSettings.breakdown.hits}
    <th class="w-12 font-normal" use:tooltip={{ content: "Total Hits" }}>Hits</th>
{/if}
{#if meterSettings.breakdown.hpm}
    <th class="w-12 font-normal" use:tooltip={{ content: "Hits per minute" }}>HPM</th>
{/if}
